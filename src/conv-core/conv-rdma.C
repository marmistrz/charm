...
void CmiNcpyBuffer::rdmaGet(CmiNcpyBuffer &source, int ackSize, char *srcAck, char *destAck) {
  ...
  NcpyOperationInfo *ncpyOpInfo = createNcpyOpInfo(source, *this, ackSize, srcAck, destAck, CMK_DIRECT_API, (void *)ref);

  CmiIssueRget(ncpyOpInfo);
}

NcpyOperationInfo *CmiNcpyBuffer::createNcpyOpInfo(CmiNcpyBuffer &source, CmiNcpyBuffer &destination, int ackSize, char *srcAck, char *destAck, int opMode, void *refPtr) {
...
void CmiNcpyBuffer::rdmaPut(CmiNcpyBuffer &destination, int ackSize, char *srcAck, char *destAck) {
  ...
  NcpyOperationInfo *ncpyOpInfo = createNcpyOpInfo(*this, destination, ackSize, srcAck, destAck, CMK_DIRECT_API, (void *)ref);
...
void zcPupGet(CmiNcpyBuffer &src, CmiNcpyBuffer &dest) {
  ...
  if(transferMode == CmiNcpyMode::MEMCPY) {
    ...
    int ackSize = 0;
    NcpyOperationInfo *ncpyOpInfo = dest.createNcpyOpInfo(src, dest, ackSize, NULL, NULL, CMK_ZC_PUP, NULL);
